...
const schema = Yup.object({
  beenHospitalized: Yup.string().oneOf(["Yes", "No"]).required(),
}).defined();

type Step13Type = Yup.InferType<typeof schema>;
...
const Step14 = ({
  ...
  const onSubmit = async (values: Step13Type) => {
    if (values) {
      action(values);
      if (nextStep) {
        setActiveStep(false);
  ...
  const options = [
    {
      value: "Yes",
      label: t("questionary:question13.options.Yes"),
    },
    {
      value: "No",
      label: t("questionary:question13.options.No"),
    },
  ];
  ...
  return (
    <MainContainer>
      ...
      <QuestionText extraSpace first>
        {t("questionary:question13.question")}
      </QuestionText>
      <Controller
        control={control}
        name="beenHospitalized"
